...
const mongoSanitize = require('express-mongo-sanitize')     // Mongo injection protector 
...
app.use(mongoSanitize())    //sanitize data for mongo injeection
...
02:
    ...
    ----------------------------------------------------------------------------------------------
            ...
            ## in google map we serach  [latitude,longitude] this way 
...
01:
                                    Adding earthquick cluster
    ----------------------------------------------------------------------------------------------
            ## https://docs.mapbox.com/mapbox-gl-js/example/cluster/ --> go to there
            ## add <div id="map"></div> to the index.ejs page where we wanna view the map

            ## Now create a cluster.js file inside public
            ## copy code from doc and paste it 

            ## in index.ejs we linked this page 
            ## before linking pass the mapToken like before


02:
                                    re seed database again
    ----------------------------------------------------------------------------------------------
            ## we now pass the random cities lattitude and longitude
...
03:
    ...
    ----------------------------------------------------------------------------------------------
            ## so we  now connect our campgrounds to the cluster map
            ## https://docs.mapbox.com/mapbox-gl-js/assets/earthquakes.geojson  --> dataset
            ## this is the API they use for data
            ## We now use our data
            ## so we need to pass our data under features key
            ## bcz its need it , this way this API designed
            ## so we stringify our campgrounds under features key in index.ejs
            ## data: campInfo --> now we set data to our object
...
04:
    ...
    ----------------------------------------------------------------------------------------------
            ## go to the 'unclustered-point' event on clusterMap.js
            ## there e.features[0] expecting properties but our campoground schema does not have it
               how mapbox works --> dataset then we saw that dataset expects (geometry , properties) under features key 
            ## We we are either use virtuals or modify schema
            ## so we decide we use virtuals

            ## Add virtuals to campground schema
            ## use it inside 
               map.on('click', 'unclustered-point', (e) => {} --> (clusterMap.js)
            ## e.features[0] now has the properties virtuals 
            ## use it 
            ...
            ## Free image sites 
                    ...
                    Freepik: https://www.freepik.com/
                                ...
                                Common Security Issue (New Folder)
    ----------------------------------------------------------------------------------------------
...
01:
    ...
    ----------------------------------------------------------------------------------------------
            ## db.users.find({username: req.body.username})
               db.users.find({username: {$gt:""}})  --> this will find all users 

               so we need to sanitize query

            ## install 'npm i express-mongo-sanitize'
            ## require it and use it as a middleware



01:
                        Coss site scripting(xss) -- >> Senitize Html with joi 
    ----------------------------------------------------------------------------------------------
            ## Hacker may include any script through our app and get access cookies and other stuff
            ## So we need to avoid accept script inside any text area 
            ## we can easily run script through our text field as we are not sanitize it 
                    <script>alert('hi')</script> write this code inside text field
            
            ## unfortunately joi dont have any build in method to sanitize it 

            ## So we need to install 'npm i sanitize-html'
            ## And write code inside schema.js to tell joi to satitize html 

            ## and extend joi with it and use a method
