...
//-----------------------------------------------------------------------------------------
//func that will reduce duplication of try catch 
...
app.get('/products/:id', wrapAsync(async (req, res, next) => {      //eita last e add kora lagbe noile products/new er shathe crash korbe
    const { id } = req.params
...
------------------------------------------------------------------------
    ...
    --> try catch is annoying to write all the time so we make func then use it so that
        we do not have to write try catch everytime
